...
const authSessionIdSchema = z
  .string()
  .trim()
  .min(8, 'sessionId is too short')
  .max(128, 'sessionId is too long')
  .regex(/^[A-Za-z0-9:_-]+$/, 'sessionId has invalid characters');

const authSessionTokenSchema = z
  .string()
  .trim()
  .min(32, 'sessionToken is too short')
  .max(2048, 'sessionToken is too long');
...
export const validateBodySchema = z
  .object({
    ...
    sessionToken: authSessionTokenSchema.optional(),
  })
  .refine(
    (value) =>
      (Boolean(value.sessionId) && Boolean(value.sessionToken)) ||
      (!value.sessionId && !value.sessionToken),
    {
      message: 'sessionId and sessionToken must be provided together',
      path: ['sessionId'],
    }
  );
...
export const resetLicenseSessionBodySchema = z.object({
